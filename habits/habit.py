...
def main():
    ...
    # Below to go in console call version
    ...
    train_bottleneck_dir = 'x'
    validate_bottleneck_dir = 'x'
    ...
    conf_object = Configuration(train_directory=train_directory,validate_directory=validate_directory,test_directory=test_directory,train_bottleneck_dir=train_bottleneck_dir,
                       ...
                       regenerate_training_inputs =regenerate_training_inputs,regenerate_test_inputs=regenerate_test_inputs,batch_size=batch_size
                       )
    ...
    if (conf_object.do_scratch_training):
        if (conf_object.regenerate_training_inputs):
            ...
            try:
                ...
                train_out_folder,train_count = create_numpy_batches(file_dir= conf_object.train_directory,out_dir=conf_object.train_directory,label_count=num_labels,
                ...
                valid_out_folder,valid_count = create_numpy_batches(file_dir=conf_object.validate_directory, out_dir=conf_object.validate_directory,
        ...
        if (conf_object.regenerate_test_inputs):
            ...
            # Create test batches
            # Existing numpy batches will be erased and replaced
            test_out_folder, test_count = create_numpy_batches(file_dir=conf_object.test_directory,
        ...
        print ('Params are; train folder: ' + str(train_out_folder) + ' valid folder: ' + str(valid_out_folder) + ' number train ' + str(train_count) + ' number validate ' + str(valid_count))
        print ('NUmber Labels:' + str(num_labels))

        nfft = int(conf_object.nfft / 2 + 1)

        model.base_train(ncep=number_cepstrums,nfft=nfft,max_len=cutoff_spectogram,label_count=num_labels,isTraining=True,batch_size=batch_size,
                         train_folder=train_out_folder,validate_folder=valid_out_folder,n_train = train_count,n_valid=valid_count
                         ,chkpoint_dir = conf_object.checkpoint_dir)
