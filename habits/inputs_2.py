...
class InputRaw(object):
    ...
    def prepare_mfcc_spectogram(self,file_dir,file_name,conf_object,mfcc_padding_value = 0,specto_padding_value = 0):


        fs,signal = wav.read(file_dir + file_name)
        mfcc = pspeech.mfcc(signal=signal,samplerate=fs,numcep=conf_object.ncep)
        f, t, specgram = sig.spectrogram(x=signal,fs=fs,nfft=conf_object.nfft)

        # Truncate mfcc frames to specified maximum cutoff
        if(mfcc.shape[0] > conf_object.cutoff_mfcc):
                mfcc = mfcc[:conf_object.cutoff_mfcc,:]

        # MFCC: Apply padding if frame length lower than cutoff
        mfcc_padding = ((0, conf_object.cutoff_mfcc - mfcc.shape[0]), (0, 0))
        ...
        if (specgram2.shape[0] > conf_object.cutoff_spectogram):
            specgram2 = specgram2[:conf_object.cutoff_spectogram,:]

        specgram_padding = ((0,conf_object.cutoff_spectogram - specgram2.shape[0]),(0,0))
    ...
    def create_randomized_bottleneck_batches(self,file_dir,conf_object):
    ...
    def create_numpy_batches(self,file_dir,conf_object):
        ...
        for file in glob.glob("*.wav"):
            ...
            mfcc,spectogram = self.prepare_mfcc_spectogram(file_dir = file_dir,file_name=file,conf_object= conf_object)
